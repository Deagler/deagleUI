...
DUI::draw::Point inputmanager::backend::mouse;
...
const KeyData inputmanager::backend::ProcessKeyboardMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	k.processed = false;
	k.modifier = Modifiers::None;
	k.scancode = (lParam >> 16) & ~(1 << 7);
	...
	if (GetKeyState(static_cast<int>(Key::Shift)) < 0)
		k.modifier |= Modifiers::Shift;
	if (GetKeyState(static_cast<int>(Key::Control)) < 0)
		k.modifier |= Modifiers::Control;
	if (GetKeyState(static_cast<int>(Key::Alt)) < 0)
		k.modifier |= Modifiers::Alt;
